...
class PasswordGenerator(QMainWindow):
    ...
    def do_when_password_edit(self) -> None:
        ...
        self.ui.line_password.textEdited.connect(self.set_strenght)
    ...
    def set_password(self) -> None:
        try:
            self.ui.line_password.setText(
                password.create_new(length=self.ui.slider_lenght.value(), characters=self.get_characters())
            )
        except IndexError:
    ...
    def set_strenght(self) -> None:
        ...
        for strenght in password.StrengthToEntropy:
            if password.get_entropy(lenght, char_num) >= strenght.value:
                self.ui.label_stenght.setText(f'Strenght: {strenght.name}')
